class_name OrtGoobytheke
extends OrtScene
## GOOBYTHEKE — Apotheke (Doc E §2.3/§2.4): Hilde am Tresen, Gläser-Regal.
## Gooby-Tropfen NUR mit Rezept-Flag (der Dialog löst das Rezept ein, das
## Laden-Sortiment verlangt es für den Direktkauf).
## G7-P55b „Läden lebendig, Teil 2“: Medikamenten-Regalwand, Wartebänkchen,
## 2 leise Kunden (OrtLeben) und Hildes Kassen-Verhalten — Käufe über das
## Basisklassen-Sheet piepen via `OrtScene._on_kasse_kunde_zahlt`.

const INNEN := "res://assets/city/innen"
const MOEBEL := "res://assets/furniture"


func _baue_innenraum() -> void:
	# Basisgrößen: Counter 2 m, Gläser 0,5×0,75 m — Skalen klein (s. rehwei.gd).
	_prop("%s/kitchencounter_straight.gltf" % INNEN, Vector3(0.0, 0.0, -1.2), 90.0, 0.9)
	_prop("%s/jar_A_large.gltf" % INNEN, Vector3(-3.2, 0.0, -3.2), 0.0, 1.6)
	_prop("%s/jar_A_large.gltf" % INNEN, Vector3(-2.2, 0.0, -3.4), 12.0, 1.3)
	_prop("%s/kitchencounter_sink.gltf" % INNEN, Vector3(3.4, 0.0, -3.0), 0.0, 0.9)
	_prop("%s/menu.gltf" % INNEN, Vector3(-4.8, 0.0, -2.6), 25.0, 1.6)
	# G7-P55b Regal-Nachschlag: Medikamenten-Regalwand hinter dem Tresen,
	# Tropfen-Gläser AUF dem Tresen, Wartebänkchen + Grünpflanze rechts.
	_prop("%s/bookcaseOpen.glb" % MOEBEL, Vector3(0.8, 0.0, -3.6), 0.0, 1.3)
	_prop("%s/bookcaseOpenLow.glb" % MOEBEL, Vector3(2.2, 0.0, -3.6), 0.0, 1.3)
	_prop("%s/jar_A_large.gltf" % INNEN, Vector3(0.0, 0.85, -0.6), 25.0, 0.5)
	_prop("%s/jar_A_large.gltf" % INNEN, Vector3(0.05, 0.85, -1.75), -20.0, 0.5)
	_prop("%s/chairCushion.glb" % MOEBEL, Vector3(5.2, 0.0, 0.2), -90.0, 1.0)
	_prop("%s/chairCushion.glb" % MOEBEL, Vector3(5.2, 0.0, 1.2), -90.0, 1.0)
	_prop("%s/pottedPlant.glb" % MOEBEL, Vector3(5.6, 0.0, -1.4), 0.0, 1.1)


func _dialog_pfad() -> String:
	return "res://scripts/city/data/dialoge/goobytheke.json"


func _sortiment_pfad() -> String:
	return CitySortiment.GOOBYTHEKE_PFAD


func _npc_konfig() -> Dictionary:
	return {"tint": Color("#4FBF8B"), "emotion": "neutral", "pos": Vector3(0.0, 0.0, -2.2)}


## G7-P55b: Ambient-Leben — 2 leise Kunden (Apotheke!), Hilde bekommt das
## Kassen-Verhalten und das Tür-Glöckchen; bewusst KEIN Gemurmel.
func _leben_konfig() -> Dictionary:
	return {
		"besucher": 2,
		"punkte":
		[
			Vector3(-3.0, 0.0, -1.2),
			Vector3(-1.6, 0.0, 1.0),
			Vector3(1.6, 0.0, -2.3),
			Vector3(3.2, 0.0, 0.6),
		],
		"sprueche": "apotheke",
		"blick": Vector3(0.0, 0.0, -4.0),
		"tuer_glocke": true,
		"kasse": true,
	}
