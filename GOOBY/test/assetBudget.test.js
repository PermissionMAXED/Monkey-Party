...
const FEATURE_CAPS_MB = {
  ...
  // V5/STICKERS wave 1: +20 sticker PNGs (each ≤ 150 KB — stickers.test.js
  // enforces the per-file cap) → dir cap 4.2 → 7.5 MB. V6/F1 (PLAN6 §5):
  // +36 themed-page PNGs (84 regular + secret; the V6 batch is quantized to
  // ≤ 96 KiB per file, asserted below) → dir cap 7.5 → 10.0 MB.
  stickers: 10.0,
  ui: 0.5,
...
test('§D7/§E-G50 per-feature caps — characters ≤ 11, stickers ≤ 10, ui ≤ 0.5, splats ≤ 33, music ≤ 30, itch ≤ 12, covers ≤ 3, acui ≤ 2.5, fonts ≤ 0.3 MB', () => {
...
// ---------------------------------------------------------------------------
// V6/F1 (PLAN6 §5 hard budget guardrail): the 36 V6-batch sticker PNGs are
// quantized to ≤ 96 KiB EACH (the legacy 49 keep the 150 KB §C5.2/§D6
// ceiling — stickers.test.js asserts that blanket cap for every file).
// The V6 batch is identified through the catalog: only V6/F1 rows carry a
// `page` id.
// ---------------------------------------------------------------------------

test('V6/F1 §5: every V6-batch sticker PNG is ≤ 96 KiB (legacy 150 KB ceiling stays)', async () => {
  const { STICKERS } = await import('../src/data/stickers.js');
  const v6 = STICKERS.filter((s) => s.page);
  assert.equal(v6.length, 36, 'the 36 V6/F1 themed-page rows');
  for (const s of v6) {
    const abs = path.join(ASSETS, 'stickers', `${s.id}.png`);
    assert.ok(fs.existsSync(abs), `missing stickers/${s.id}.png`);
    const bytes = fs.statSync(abs).size;
    assert.ok(bytes <= 96 * 1024, `${s.id}.png is ${bytes} B (> 96 KiB — PLAN6 §5)`);
  }
});
